<script lang="ts">
  ...
  import { X, Plus, Trash2, RefreshCw } from 'lucide-svelte';
  ...
  async function handleLoadModels() {
    ...
    try {
      const models = await APIService.fetchModels(newConfig.baseURL, newConfig.apiKey);
  ...
  function handleAddConfig() {
    if (!newConfig.name || !newConfig.baseURL || !newConfig.apiKey || newConfig.models.length === 0) {
      ...
      <div class="modal-body">
        ...
        <div class="config-list">
          ...
          {#each $apiConfigStore.configs as config (config.id)}
            <div class="config-item {$apiConfigStore.selectedConfigId === config.id ? 'active' : ''}">
              <div class="config-info" on:click={() => handleSelectConfig(config.id)}>
